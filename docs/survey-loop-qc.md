...
4. Subtract the chord vector; **|뿯ν|** is the 3D misclosure magnitude (metres).
...
- `hDist = distance 뿯½ cos(clino뿯½)`
- `dE = hDist 뿯½ sin(azimuth뿯½)`
- `dN = hDist 뿯½ cos(azimuth뿯½)`
- `dZ = distance 뿯½ sin(clino뿯½)`
...
**ppm** = `(misclosureM / pathLengthM) 뿯½ 1_000_000` (omit when path length is 0).
...
- **With loops:** worst |뿯ν|, severity tier, worst station cycle (`A뿯↽B뿯↽C뿯↽A`), path length, optional ppm; when multiple loops, total |뿯ν| across all loops.

## Implementations

| Platform | Detection | Summary UI |
|----------|-----------|------------|
| **Web** | `src/utils/surveyLoopQc.js` | Workspace QC panel, Cave AI `survey_qc_summary` |
| **Android** | `SurveyLoopQcSummary.kt` 뿯↽ `analyzeSurveyLoops` | Offline assistant, exports, telemetry |
| **Windows** | `Services/SurveyLoopQc/SurveyLoopQcAnalyzer.cs` | SURVEY QC tab (read-only); Loop closure assistant uses separate adjuster |

## Parity tests

Shared fixture: [test-fixtures/survey-loop-qc-fixture.json](./test-fixtures/survey-loop-qc-fixture.json)

- Web: `npm run test:survey-loop-qc`
