...
public static class ProjectAiAssetPersistence
{
        ...
        byte[]? structureMaskPng)
    {
        ...
        var baseDir = Path.GetDirectoryName(Path.GetFullPath(sourceFilePath));
        if (string.IsNullOrEmpty(baseDir))
            return;

        var caveFolder = SanitizeFolderName(project.Name);
        var assetRoot = Path.Combine(baseDir, "export_assets", "windows", caveFolder);
        ...
        if (aiCartographyPng is { Length: > 0 })
        {
            ...
            File.WriteAllBytes(Path.Combine(assetRoot, fileName), aiCartographyPng);
            var rel = ToForwardSlash(Path.Combine("export_assets", "windows", caveFolder, fileName));
        ...
        if (structureMaskPng is { Length: > 0 })
        {
            ...
            File.WriteAllBytes(Path.Combine(assetRoot, fileName), structureMaskPng);
            var rel = ToForwardSlash(Path.Combine("export_assets", "windows", caveFolder, fileName));
